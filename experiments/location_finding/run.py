...
def parse_args():
    ...
    p.add_argument("--final_L", type=int, default=None, help="Contrastive samples for final eval")
    # Step-DAD
...
def main():
    ...
    if args.mode == "random":
        ...
        if args.mode == "stepdad":
            print(f"\nRunning Step-DAD with τ={args.tau}, {args.n_finetune_steps} fine-tune steps ...")
            theta_true = prior.sample(1).to(device)  # [1, K*p]
            print(f"True theta: {theta_true.squeeze().tolist()}")
